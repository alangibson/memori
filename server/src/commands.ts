...
export class Commands {
    ...
    async log(command: ICommand) {
        ...
        console.debug(`Commands.log() : Logging command action ${command.action} for ${command.encodingFormat} ${command.url}`);
    ...
    async remove(toRemove: ICommand | URL) {
        ...
        if (toRemove instanceof URL)
            ...
            await this.replay(async (command: ICommand) => {
                if (command.url == toRemove)
                    this.remove(command);
            });
        else
            if (toRemove.commandId) {
                console.debug(`Commands.remove() : Removing command log item ${this.path}/${toRemove.commandId}`);
                await fs.rm(`${this.path}/${toRemove.commandId}`);
            } else {
                throw new Error(`Can't remove logged command with undefined commandId`);
            }        
    }
