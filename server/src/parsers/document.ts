...
export class DocumentParser implements IParser {
    ...
    async parse(response: ICommittable): Promise<IMemory[]> {
        ...
        console.debug(`DocumentParser.parse() : Parsing blob of size ${response.blob.length}`);

        // Extract all text from document
        const allText: string = await new Promise((resolve, reject) => {
            textract.fromBufferWithMime(response.encodingFormat, response.blob,
        ...
        let partialMemory: Partial<IMemory> = await new Promise((resolve, reject) => {
            getDocumentProperties.fromBuffer(response.blob, (error: Error, data: any) => {
                if (error) {
                    reject(error);
                } else {
