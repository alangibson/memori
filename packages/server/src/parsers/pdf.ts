...
export class PdfParser implements IParser {
    async parse(response: ICommittable): Promise<IMemory[]> {
        ...
        const pdf: PDFExtractResult = await new PDFExtract().extractBuffer(
            response.blob
        );
        ...
        pdf.pages.forEach((page) =>
            page.content.forEach((content) => (text += content.str + ' '))
        );
        ...
        return [
            {
                ...
                '@id': atId,
                name:
                    pdf.meta?.info?.Title || response.name || text.slice(0, 50),
                url: response.url,
                text: text,
