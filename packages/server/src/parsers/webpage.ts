...
function extractMainEntityOfPageUrlFromSchema(
    schema: IRawSchemaOrg
): URL | undefined {
    if (schema.mainEntityOfPage?.['@id'])
        try {
            ...
            console.debug(
                `extractMainEntityOfPageUrlFromSchema() : Couldn't turn ${schema.mainEntityOfPage?.['@id']} into a URL`
            );
        }
    // else undefined
}
...
function makeIdUrl(schema: IRawSchemaOrg, microdata: IMicrodata): URL {
    ...
    else return cidUrl(Buffer.from(JSON.stringify(schema)));
}
...
function extractTextFromDocument(doc: Document): string {
    ...
    if (text == null) throw new Error(`Could not extract text`);
    return text;
}

// Extract all indexable text from schema, but not from it's children
function textFromSchema(schema: object) {
    // @ts-ignore because we are using conditions everywhere
    return `${schema.text || ''} ${schema.name || ''} ${
        // @ts-ignore because we are using conditions everywhere
        schema.description || ''
        // @ts-ignore because we are using conditions everywhere
    } ${schema.headline || ''} ${
...
export async function extractMicrodataFromHtml(
    html: string
): Promise<IMicrodata> {
    ...
    data.schemaOrg =
        data?.schemaOrg?.items.map((schema: ISchemaish): IRawSchemaOrg => {
    ...
    else data.schemaOrg.push(data.jsonLd);
...
export class WebPageParser implements IParser {
    ...
    async extract(microdata: IMicrodata): Promise<IMemory[]> {
        ...
        try {
            var urlBase: string = new URL(microdata.general.canonical || '')
                .origin;
        } catch (e) {
            var urlBase = '';
        }
            ...
            schemas = schemas.concat(
                microdata.schemaOrg.map((schema: IRawSchemaOrg): IMemory => {
                    ...
                    const urlString: string | undefined =
                        schema.url || schema.embedUrl || schema['@id'];
                    ...
                    if (urlString) {
                        ...
                        if (schema.encoding && schema.encoding?.length > 0) {
                            if (!url) {
                                ...
                                console.debug(
                                    `Content URL string is ${schema.encoding[0].contentUrl}`
                                );
                                url = new URL(
                                    schema.encoding[0].contentUrl,
                                    urlBase
                                );
                    ...
                    return {
                        ...
                        url:
                            url ||
                            extractMainEntityOfPageUrlFromSchema(schema) ||
                            makeIdUrl(schema, microdata),
                        name:
                            schema.name ||
                            schema.title ||
                            schema.headline ||
                            makeIdUrl(schema, microdata).toString(),
    ...
    async parse(response: ICommittable): Promise<IMemory[]> {
        ...
        let webPageIndex: number = extractedSchemas.findIndex(
            (schema) => schema['@id'] == response.url.toString()
        );
        if (webPageIndex >= 0) {
            ...
            var webPage: IMemory = {
                ...
                abstract:
                    microdata?.general?.description?.trim() ||
                    abstractFromString(text),
        ...
        webPage['m:embeddedIds'] = extractedSchemas.map(
            (memory: IMemory): IdRef => ({ '@id': memory['@id'] })
        );
