...
export class VideoEnhancer implements IEnhancer {
    ...
    private async download(
        ...
        // Figure out filename
        const basename = [
            Buffer.from(media.url + media['@type']).toString('base64url')
        ];
        ...
        console.log(
            `VideoSchemaEnhancer.enhance() : Performing speach-to-text on ${media.url}`
        );
    ...
    async thumbnail(url: string): Promise<[{}, {}]> {
        const rememberable: IRememberable = await new Fetcher().fetch(
            new URL(url)
        );
        let thumbnailSchema = {};
    ...
    async enhance(media: IMemory): Promise<IMemory> {
        ...
        console.log(
            `VideoSchemaEnhancer.enhance() : Getting video ${media.url}`
        );
