...
export class Enhancer implements IEnhancer {
    ...
    async enhance(media: IMemory): Promise<IMemory> {
        if (media['@type'] == 'WebPage')
            return new WebPageEnhancer().enhance(media);
        else if (media['@type'] == 'VideoObject')
            return new VideoEnhancer(
                `${process.cwd()}/etc/${this.settings.voskModel}`
            ).enhance(media);
        else if (media['@type'] == 'ImageObject')
            return new ImageEnhancer(this.settings.ocrLanguage).enhance(media);
