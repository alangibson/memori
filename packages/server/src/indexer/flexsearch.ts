...
export class FlexsearchSearch implements ISearchIndex, IPersistable {
    ...
    async remove(id: URL) {
        try {
            ...
            console.warn(
                `FlexsearchSearch.remove() : Failed to remove @id ${id} because ${e}`
            );
        }
    }
    ...
    async save() {
        ...
        this.index.export(async (key: string | number, data: string) => {
    ...
    async load() {
        try {
            ...
            for (const key of keys) {
                ...
                const data: Buffer | undefined =
                    index._attachments?.[index['@id']].data;
                if (data != undefined)
                    this.index.import(key, data.toString('utf8'));
                else
                    console.debug(
                        `FlexsearchSearch.load() : Couldn't retrieve index part ${id}`
                    );
            }
        } catch (e) {
