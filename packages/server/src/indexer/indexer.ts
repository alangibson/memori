...
export class Index implements IPersistable {
    ...
    async index(things: IMemory[]) {
        ...
        await Promise.all(
            things.map(async (storable: IMemory) => {
                console.debug(
                    `Index.store() : Upserting ${storable['@type']} ${storable['@id']}`
                );
                try {
                    await this.db.upsert(
                        ...
                        (existing: {}): IMemory => {
                            console.log(
                                // @ts-ignore because _rev can be undefined
                                `Previous ${existing._rev}, new : ${storable._rev}`,
                                Object.keys(storable).includes('_rev')
                            );
                            console.log(
                                `Attachments in ${storable['@id']}`,
                                storable._attachments
                            );
    ...
    async getById(
        ...
        if (embedded && memory['m:embeddedIds']) {
            console.debug(
                `Index.getById() : Hydrating ${memory['m:embeddedIds'].length} embedded documents in id: ${id}`
            );

            // getById() for each IdRef in memory['m:embeddedIds']
            // TODO use this.hydrateIdRefs()
            const x = await Promise.all(
                memory['m:embeddedIds']?.map(
                    async (idRef: IdRef): Promise<IMemory | undefined> => {
                        try {
    ...
    async search(q: string): Promise<IRecalledMemory[]> {
            ...
            searchResults
                ...
                .map(async (id: URL): Promise<IRecalledMemory | undefined> => {
                    ...
                    try {
                        ...
                        console.warn(
                            `Index.search() : Found ${id} in search index, but not in db`
                        );
                        return undefined;
                    }
                })
        ...
        const filteredRecalled: IRecalledMemory[] = <IRecalledMemory[]>recalled
            ...
            .filter(
                (recalled: IRecalledMemory | undefined): boolean =>
                    recalled != undefined &&
    ...
    private async hydrateIdRefs(
        idRefs: IdRef[] | undefined
    ): Promise<IMemory[]> {
        console.debug(`hydrateIdRefs() : Hydrating ${idRefs?.length} id refs`);
        if (idRefs == undefined || idRefs.length <= 0) return [];
        return await Promise.all(
            idRefs.map((idRef: IdRef) => this.getById(new URL(idRef['@id'])))
        );
    }
    ...
    async all(
        ...
        const memories: IRecalledMemory[] = await Promise.all(
            found
                ...
                .map(async (memory: IMemory): Promise<IMemory> => {
                    memory['m:embedded'] = await this.hydrateIdRefs(
                        memory['m:embeddedIds']
                    );
                    return memory;
                })
        ...
        return (
            memories
                ...
                .filter(
                    (doc: IRecalledMemory) =>
                        doc.thing['m:embeddedInId'] == undefined
                )
                // Sort by lexical order of m:created
                .sort((a: IRecalledMemory, b: IRecalledMemory) =>
                    // @ts-ignore because name is a string variable
                    a.thing[sort] > b.thing[sort] ? -1 : 1
                )
        );
    }
